...
namespace Kyrsach_K3S2_V1.Services
{
    class RubasheeService
    {
        ...
        public void GetRubashee(List<Weapon> weapon, string quality)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();
                ...
                {
                    command.CommandText = $@"Select КодОружия, Тип,  Качество, ТребуетсяРазрешение, Стоимость, Описание 
                                           From ОружиеРубящее Where Качество = @Качество";
